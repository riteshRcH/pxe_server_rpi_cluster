...
if __name__ == '__main__':          #unit testing
    try:
        ...
        parser.add_argument('-hp', '--http-server-port', action = 'store', dest = 'HTTP_SERVER_PORT', help = 'HTTP Server Port num', default = HTTP_SERVER_PORT)
        parser.add_argument('-iso', '--iso-filename', action = 'store', dest = 'ISO_FILENAME', help = 'ISO FILE NAME TO BOOT using iPXE', default = ISO_FILENAME)
        ...
        if args.USE_HTTP:
            ...
            httpServer = HTTPServer.HTTPD(enableVerboseOutput = args.MODE_VERBOSE)
        ...
        """
        if args.USE_HTTP:
            f = open('boot.http.ipxe', 'w')
            f.write('#!ipxe\n')
            f.write('dhcp\n')
            f.write('initrd http://'+str(args.DHCP_FILESERVER)+':'+str(args.HTTP_SERVER_PORT)+'/'+args.ISO_FILENAME+'\n')
            f.write('chain http://'+str(args.DHCP_FILESERVER)+':'+str(args.HTTP_SERVER_PORT)+'/memdisk iso')
            f.close()
